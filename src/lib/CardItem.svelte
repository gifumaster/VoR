<script lang="ts">
  ...
  async function handleCardSelect() {
    ...
    // 選択されたカードをストアに追加
    selectedCardStore.update((cards) => [
      ...cards,
      {
        name,
        addAbillity,
...
>
  ...
  <table class="stats">
    <tbody>
      ...
      {#if addBulletAmountPerShot !== undefined && addBulletAmountPerShot < 0}
        <tr
          ><td>同時発射数:</td><td class="negative">{addBulletAmountPerShot}</td
          ></tr
        >
      {/if}
      {#if addFireRate !== undefined && addFireRate < 0}
      ...
      {#if addMovingSpeedRate !== undefined && addMovingSpeedRate < 0}
        <tr
          ><td>移動速度:</td><td class="negative">{addMovingSpeedRate}%</td></tr
        >
      {/if}
      {#if addJumpHeightRate !== undefined && addJumpHeightRate < 0}
        <tr
          ><td>ジャンプ力:</td><td class="negative">{addJumpHeightRate}%</td
          ></tr
        >
      {/if}
      {#if addNumberOfJump !== undefined && addNumberOfJump < 0}
        <tr
          ><td>ジャンプ回数:</td><td class="negative">{addNumberOfJump}</td></tr
        >
      {/if}
      {#if addReloadSpeedRate !== undefined && addReloadSpeedRate > 0}
        <tr
          ><td>リロード速度:</td><td class="negative">{addReloadSpeedRate}%</td
          ></tr
        >
      {/if}
      {#if addReloadSpeedSeconds !== undefined && addReloadSpeedSeconds > 0}
        <tr
          ><td>リロード速度:</td><td class="negative"
            >{addReloadSpeedSeconds} sec</td
          ></tr
        >
      {/if}
      {#if addShieldCoolTimeRate !== undefined && addShieldCoolTimeRate > 0}
        <tr
          ><td>シールドCD:</td><td class="negative">{addShieldCoolTimeRate}%</td
          ></tr
        >
      {/if}
      {#if addShieldCoolTimeSeconds !== undefined && addShieldCoolTimeSeconds > 0}
        <tr
          ><td>シールドCD:</td><td class="negative"
            >{addShieldCoolTimeSeconds} sec</td
          ></tr
        >
      {/if}
    </tbody>
